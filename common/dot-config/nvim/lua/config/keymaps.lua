...
function M.csd_snips()
    ...
    map({ "n" }, "<leader>ddc", function()
        csd_snips.vars_to_console()
    end, { desc = "Write debug message for STDOUT (csd-snips)" })
    map({ "n" }, "<leader>ddl", function()
        csd_snips.vars_to_logger()
    end, { desc = "Write debug message for logger (csd-snips)" })
    map({ "n" }, "<leader>dcs", function()
        csd_snips.vars_string_to_console()
    end, { desc = "Write debug message for STDOUT (csd-snips)" })
    map({ "n" }, "<leader>dls", function()
        csd_snips.vars_string_to_logger()
    end, { desc = "Write debug message for logger (csd-snips)" })
    map({ "n" }, "P", function()
        csd_snips.word_under_cursor_to_console()
    end, { desc = "Write word under cursor to STDOUT (csd-snips)" })
    map({ "n" }, "L", function()
        csd_snips.word_under_cursor_to_logger()
